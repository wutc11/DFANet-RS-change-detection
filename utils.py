...
def createTrainingCubes(X, y, patch_size):
    ...
    # 筛选标签全部训练
    pdata = np.zeros((ele_num1+ele_num2, patch_size, patch_size, X.shape[2]))
    plabels = np.zeros(ele_num1+ele_num2)
    for i in range(patchesData_1.shape[0]):
        pdata[i, :, :, :] = patchesData_1[arr_1[i], :, :, :]
        plabels[i] = patchesLabels_1[arr_1[i]]
    for j in range(patchesData_2.shape[0]):
        pdata[patchesData_1.shape[0]+j, :, :, :] = patchesData_2[arr_2[j], :, :, :]
        plabels[patchesData_1.shape[0]+j] = patchesLabels_2[arr_2[j]]
